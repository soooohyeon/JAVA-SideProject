...
public class UserDAO {
	...
	public int register(UserVO userVO) {
		...
		/* 메인 메소드에서 사용 */
		if(checkID(userVO.getUserId()) != 0) {return -1;} // 테이블에서 중복되는 아이디를 찾아 회원번호를 저장한 경우
	...
	public int checkID(String userId) {
		...
		int userNumber = 0; // 아이디가 존재하는 경우 해당 회원의 회원번호를 저장할 변수
		...
		try {
			...
			if(resultSet.next()) {userNumber = resultSet.getInt(1);} // 테이블에서 중복되는 아이디를 찾은 경우
		...
		return userNumber; // 0은 회원번호를 찾지 못했을 때, 회원번호를 찾았을 때는 1 이상의 값을 저장하게 됨 
	}
	...
	public boolean countPhone(String userPhoneNumber) {  
		...
		try {
			...
			if(resultSet.next()) {cntPhone = resultSet.getInt(1);} // 매개변수로 받은 휴대폰번호가 테이블에서 조회가 되는 경우 해당 휴대폰 번호의 아이디 개수를 저장
			if(cntPhone > 2) { return false;} // 휴대폰 번호로 가입된 계정이 3개가 넘을 경우 false 즉, 더이상 회원가입 불가
	...
	public int login(String userId, String userPassword) {
		...
		try {
			...
			if(resultSet.next()) {loginStatus = resultSet.getInt(1);} // 만약 입력한 아이디와 비밀번호가 모두 일치하는 회원을 찾은 경우 해당 회원번호를 전역변수에 있는 loginStatus에 저장
	...
	public UserVO selectUser() {
		String query = "select userNumber, userId, userPassword, userName, userPhoneNumber, userLikeTeam, userLikePlayer, userPoint from tbl_user "
    ...
    public String findTeamName() {
        ...
        try {
            ...
            resultSet = preparedStatement.executeQuery();
            if(resultSet.next()) {teamName = resultSet.getString(1);} // 쿼리문의 결과가 있는 경우 팀이름을 teamName 변수에 저장
    ...
    public String findPlayerInfo() {
        ...
        String query = "SELECT E1.Tname FROM "
        		...
        		+ "where tu.userNumber = ?";
        ArrayList<String> list = new ArrayList<String>(); // 회원의 정보를 저장할 배열
	...
	public String findTeamName(int teamNum) {
        // 로그인 한 회원이 좋아하는 팀 번호를 받아, 프리미어리그 팀의 번호와 이름이 저장되어 있는 tbl_favoriteTeam 테이블에서 팀번호와 일치하는 팀의 이름 검색
        String query = "select ts.userTeamName from "
        		+ "tbl_user tu join tbl_soccerteam ts "
        		+ "on tu.userLikeTeam = ts.userTeamNumber "
        		+ "where tu.userLikeTeam = ?";
        String teamName = null;

...
//         String todaySoccer = "20220901";
            ...
            if (date.contains(todaySoccer)) {		// 경기가 있다면
                for (int i = 0; i < date.size(); i++) {
                    if (todaySoccer.equals(date.get(i))) {
                        System.out.println(time.get(i) + "   장소 : " + place.get(i) + "     "+ homeTeams.get(i) + "  VS  " + awayTeams.get(i));                        
                    }
                }
            } else {		// 경기가 없다면
...
//                
            ...
            for (int j = 0; j < homeTeams.size(); j++) {
                if (teamName.equals(homeTeams.get(j)) || teamName.equals(awayTeams.get(j))) {
                    result += ++i + ". 날짜 : " +
                            dates.get(j) + " / 시간 : " + times.get(j) + " / 구장 : " + stadiums.get(j) + " / " + homeTeams.get(j)
                            + " " + homeScores.get(j) + " : " + awayScores.get(j) + " " + awayTeams.get(j) + "\n";
                }
            }
		...
		ArrayList<String> awayScores = new ArrayList<String>();
			...
			for (int j = 0; j < homeTeams.size(); j++) {
				if (!homeScores.get(j).equals("-")) {
					results.add(dates.get(j) + "," + homeTeams.get(j)
					 + "," + homeScores.get(j) + "," + awayScores.get(j) + "," + awayTeams.get(j));
				}
			}
			Random r = new Random();
